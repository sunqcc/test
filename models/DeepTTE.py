...
class EntireEstimator(nn.Module):
    ...
    def eval_on_batch(self, pred, label, mean, std):
        ...
        mae_loss = torch.mean(torch.abs((pred - label)*60))
        rmse_loss = torch.sqrt(torch.mean(((pred - label)*60)**2))

        return {'label': label, 'pred': pred}, loss.mean(),mae_loss,rmse_loss
...
class Net(nn.Module):
    ...
    def eval_on_batch(self, attr, traj, config):
        ...
        pred_dict, entire_loss,mae,rmse = self.entire_estimate.eval_on_batch(entire_out, attr['time'], config['time_mean'], config['time_std'])
        ...
        if self.training:
            ...
            return pred_dict, entire_loss,mae,rmse
